...
Uso:
    ros2 launch nav2_trekking nav2.launch.py map:=$HOME/Workspace/Controle_robo_web/maps/meu_mapa.yaml
"""
...
def generate_launch_description():
    ...
    # 2026-08-27 (nav2_trekking): o nav2_params_legacy.yaml foi APAGADO deste
    # pacote. Ele carregava a stack de 06-08 (NavFn, DWB sem shim, PolygonSlow
    # viva) — passar params_file apontando pra ele ressuscitaria em silêncio
    # justamente o medo que este pacote existe pra não ter.
    ...
    nodes = [
        ...
        Node(
            ...
            # do mux (auto_vel_raw) é filtrada pelo collision_monitor (->auto_vel).
        ...
        Node(
            package='nav2_trekking', executable='scan_sanitizer',
        ...
        Node(
            package='nav2_trekking', executable='freeze_capture',
        ...
        Node(
            package='nav2_trekking', executable='path_follower',
        ...
        # Re-habilitar = descomentar este bloco (e colcon build nav2_trekking).
        # Node(
        #     package='nav2_trekking', executable='door_crossing',
        ...
        # que entra direto no collision_monitor. unstuck/humano NÃO entram aqui
        # — ficam no mux FINAL (robot/sim.launch.py), a jusante do collision.
        # 2026-08-27 (nav2_trekking): o motion_guard (cautela com pessoas) foi
        # REMOVIDO deste pacote; o estágio auto_vel_pre -> auto_vel_raw que ele
        # ocupava deixou de existir e o mux publica auto_vel_raw diretamente.
        Node(
        ...
        Node(
            package='nav2_trekking', executable='unstuck_supervisor',
            name='unstuck_supervisor', output='screen',
            parameters=[sim_time_param, {
                # 2026-08-27 (pedido do dono: "quase instantâneo"). Os 5.0 s
                # eram a espera CAUTELOSA de quando o bloqueio podia ser uma
                # PESSOA — não há mais pessoas nesta stack, então esperar 5 s
                # parado é tempo jogado fora numa pista.
                # 5.0 -> 2.0 s (não 0.5). MEDIDO: com 0.5/0.3 o unstuck cortava
                # POINT-TURN LEGÍTIMO — girando no lugar o robô não translada, e
                # embora o gate stuck_yaw (9°) cubra o giro que ANDA, o giro que
                # está sendo engolido (comando abaixo da zona-morta) parece
                # encalhe de verdade. Resultado da volta com 0.5/0.3: 217 s de
                # manobra do unstuck e 2 goals perdidos. 2.0 s dá folga pra um
                # point-turn inteiro (180° a 2.4 rad/s ≈ 1.3 s) terminar.
                'stuck_timeout': 2.0,
                # Recovery contextual (2026-06-22): bloqueio à frente que bate no
                # /map cru (parede mapeada) dispara ANTES do desconhecido (era
                # 2s vs 5s; 2026-08-27 virou 1.0s vs 2.0s). Ver
                # docs/superpowers/specs/2026-06-22-unstuck-recovery-contextual-design.md
                'stuck_timeout_mapped': 1.0,
                'block_range': 0.5,
                ...
                'rear_lidar_x': 0.0,
                'rear_tail_x': -0.25,
                # 2026-08-27: 0.30 -> 0.26. Esta meia-largura define o CORREDOR
                # que o unstuck considera "à minha frente" (front_min_gap: pontos
                # com x>head_x e |y|<=half_width) e "atrás". Com 0.30 uma parede
                # LATERAL a 0.275 m caía dentro da faixa -> o unstuck lia
                # "front_gap = 0.05 m" (frente bloqueada) sem ter nada na frente,
                # e escolhia GIRAR pro lado aberto — a pior manobra ali, porque o
                # canto do robô varre 0.354 m contra uma parede a 0.275.
                # MEDIDO no sim (rota1/sala_grande): 26 disparos, 21 turning com
                # front_gap 0.00-0.09 enquanto o scan não via NADA a menos de
                # 0.41 m à frente. Com 0.26 (1 cm além do corpo) a parede a 0.275
                # não entra mais na faixa: a frente lê livre e o unstuck escolhe
                # AVANÇAR, que é a regra que já existia ("frente livre = o
                # caminho; dar ré aqui desfaz o progresso"). Reverter = 0.30.
