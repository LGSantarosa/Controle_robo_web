...
from nav2_trekking.unstuck_supervisor import (
    UnstuckConfig,
    UnstuckSupervisor,
...
from nav2_trekking.unstuck_supervisor import door_zone_active
...
from nav2_trekking.unstuck_supervisor import MapGrid, map_occupied
...
from nav2_trekking.unstuck_supervisor import front_block_point, block_point_mapped
...
def _pinched_scene(sup, t, rear=0.0):
    # cena do campo 07-10: FRENTE livre, robô pinçado de lado.
    # O unstuck deve escolher AVANÇAR (a frente livre é o caminho), não ré.
    return sup.update(t, nav_wants_move=True, position=(0.0, 0.0),
                      rear_gap=rear, front_gap=math.inf, side_clear=0.2,
                      nearest=0.0)


def test_pinched_with_front_clear_advances():
    sup = UnstuckSupervisor(_cfg())
    _tick(sup, 0.0)
    _pinched_scene(sup, 0.1)
    cmd = _pinched_scene(sup, 10.4)                 # >stuck_timeout: dispara
    assert cmd.active is True and cmd.lin > 0.0     # avanço (frente livre)
