...
1) freeze_capture.csv — a CADEIA de velocidade + odom + estados, 1 linha por msg:
     ...
     auto_vel_raw : nav+seguidor+porta arbitrados pelo mux de autonomia
                    (saída do twist_mux_auto, PRÉ-collision)
     ...
     follow_state / goal_active : transições (valor na col. extra)
   → orçamento do tempo parado (07-03): `bin/pause_budget.py freeze_capture.csv`
     atribui cada segundo parado-com-goal a uma causa (collision, giro engolido,
     zona-morta, unstuck, follower quieto...) pra achar o vilão.
...
class FreezeCapture(Node):
    def __init__(self):
        ...
        for topic in ('cmd_vel_nav', 'nav_vel', 'follow_vel',
